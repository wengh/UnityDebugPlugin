            ...
            if (selecting)
            {
                ...
                if (Physics.Raycast(ray, out RaycastHit hit))
                {
                    ...
                    else if (highlighted != renderer)
                    {
                        ...
                        highlighted = renderer;
                        previousColor = highlighted.material.color;
                        highlighted.material.color = Color.red;
                    }
                    if (Input.GetMouseButtonDown(0))
                    {
                        SelectedGameObject = highlighted.gameObject;
                        highlighted.material.color = Color.yellow;
                        selecting = false;
                    }
                }
            }
            else if (highlighted != null && SelectedGameObject != highlighted?.gameObject)
            {
                highlighted.material.color = previousColor;
            }
        }
